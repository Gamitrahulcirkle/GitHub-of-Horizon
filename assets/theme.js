...
theme.settings = theme.settings || orgTheme.settings || {};
theme.routes = theme.routes || orgTheme.routes || {};
theme.strings = theme.strings || orgTheme.strings || {};
theme.template = theme.template || orgTheme.template || {};
...
(function(){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function({srcElement}) {
        // varient change metafiled
        // console.log('change',srcElement.dataset.metafield_value)
        // document.querySelector(".meta_out_value").innerHTML = "";
        // if(srcElement.dataset.metafield_value !== undefined){
        //   document.querySelector(".meta_out_value").innerHTML = srcElement.dataset.metafield_value;
        // } 
  ...
  theme.cart = {
    ...
    _updateCart: function(params) {
      ...
      .then(cart => { 
        if (typeof window.handleFreeItems === 'function') {
          let cartObj = null;

          try {
            cartObj = JSON.parse(cart);
          } catch (e) {
            console.warn('Cart is not valid JSON:', e);
          }

          window.handleFreeItems(cartObj);
        }
        return cart; 
      });
    },
  ...
  theme.CartForm = (function() {
    var selectors = {
      ...
      subTotalOriginal: '[data-subtotal-price]',
    ...
    function CartForm(form) {
      ...
      this.subTotalOriginal = form.querySelector(selectors.subTotalOriginal);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(text) {
        ...
        var OriginalTotal = items.dataset.cartOriginalTotal;
        ...
        this.products.append(items);
           var spoon_prio = document.getElementsByClassName('spoon_prod_tag');
        // Select all elements with the class name 'scoop_prod'
        var spoon_prio_css = document.getElementsByClassName('scoop_prod');

        if (spoon_prio.length > 0) {
            // Loop through all elements with the class name 'scoop_prod' and hide them
            for (var i = 0; i < spoon_prio_css.length; i++) {
                spoon_prio_css[i].style.display = 'none';
            }
        } else {
            // Loop through all elements with the class name 'scoop_prod' and show them
            for (var i = 0; i < spoon_prio_css.length; i++) {
                spoon_prio_css[i].style.display = 'flex';
            }
        }
  
        // Update subtotal
        this.subtotal.innerHTML = theme.Currency.formatMoney(subtotal, theme?.settings?.moneyFormat);
        const original = parseInt(OriginalTotal, 10);
        const current = parseInt(subtotal, 10);

        if (this.subTotalOriginal) {this.subTotalOriginal.innerHTML = '';}
        if (original > current && this.subTotalOriginal) {
          this.subTotalOriginal.innerHTML =
            `<del>${theme.Currency.formatMoney(original, theme?.settings?.moneyFormat)}</del>`;
        }
        this.reInit();
  ...
  //       console.log('this.args', this.args.scopedEventId)
  ...
  //console.log("currentlySelectedValues",currentlySelectedValues)
  ...
  theme.headerNav = (function() {
    ...
    function toggleThumbMenu(active, forceClose) {
      ...
      var thumbBtn = document.querySelector('.slide-nav__overflow--thumb');
        ...
        if (thumbBtn && thumbBtn.classList.contains('js-menu--is-open')) {
          return;
        }
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          //  console.log("evt",evt)
          // Regular price
          if( this.container.querySelector(selectors.price) != null ){
          ...
          if (variant.compare_at_price > variant.price) {
            if( this.container.querySelector(selectors.comparePrice) != null ){
            ...
            if( this.container.querySelector(selectors.priceWrapper) != null ){
            ...
            if( this.container.querySelector(selectors.comparePriceA11y) != null ){
              this.container.querySelector(selectors.comparePriceA11y).setAttribute('aria-hidden', 'false');
            }
            if( this.container.querySelector(selectors.priceA11y) != null ){
      ...
      initImageZoom: function() {
        console.log("initImageZoom work")
  ...
  theme.HeaderSection = (function() {
    function HeaderSection(container) {
      console.log("HeaderSection fuction")
      this.initDrawers();
      this.hoverMenu();
    ...
    HeaderSection.prototype = Object.assign({}, HeaderSection.prototype, {
      initDrawers: function() {
        console.log("initDrawers fuction")
        if (theme?.settings?.cartType === 'drawer') {
          console.log("cart drawer fuction")
          if (!document.body.classList.contains('template-cart')) {
  ...
  theme.Photoswipe = (function() {
    var selectors = {
      ...
      slideshowTracked: '.product-image-main ',
    ...
    Photoswipe.prototype = Object.assign({}, Photoswipe.prototype, {
      ...
      triggerClick: function(evt) {
        ...
        if (this.container.dataset && this.container.dataset.hasSlideshow === 'true') {
          ...
          console.log("this.inSlideshow = true")
        } else {
          this.inSlideshow = false;
          console.log("this.inSlideshow = false;")
        }
      ...
      getImageData: function() {
        ...
        if( this.images.length <= 0 ){
          this.images = this.inSlideshow
                        ? this.container.querySelectorAll(selectors.slideshowTracked + selectors.images)
                        : this.container.querySelectorAll(selectors.images);
        }
        ...
        console.log("getImageData selectors",selectors)
        console.log("getImageData",items)
        return items;
      },
  
      initGallery: function(items, index) {
        console.log("initGallery items", items);
        console.log("initGallery index", index);
  ...
  DOMready(function(){
    ...
    // product page table pop-up start
  //   var openPopupLink = document.querySelector(".prod_desc_url");
  //   var productPopup = document.querySelector(".product_popup_wp");
  //   var close_ic = document.querySelector(".close_click");
  //   var pop_over = document.querySelector(".pop-ovelay");
  //   // Add click event listener to the open-popup link
  // if(openPopupLink != null) {
  ...
  // }
  //  if(pop_over != null) {
  //   pop_over.addEventListener("click", function() {
  //     productPopup.classList.remove("active");
  //   });
  //  }
...
document.addEventListener("DOMContentLoaded", function() {
  const subLabels = Array.from(document.querySelectorAll('.sub_label'));

  // Store original values
  subLabels.forEach(sl => {
    const original = sl.getAttribute('data') || sl.textContent.trim();
    sl.dataset.original = original;
  });
  ...
  function applySubscriptionDiscount() {
    subLabels.forEach(sl => {
      const original = sl.dataset.original || '';
      const parts = original.split('/');
      const price = parsePrice(parts[0]);
      if (isNaN(price)) {
        sl.textContent = original;
        return;
      }

      let discounted = price - price * 0.05;

      // ✅ Static override conditions
      if (discounted.toFixed(2) === "1.55") discounted = 1.54;
      if (discounted.toFixed(2) === "1.27") discounted = 1.28;

      sl.textContent = '£' + discounted.toFixed(2) + (parts[1] ? '/' + parts[1] : '');
    });
  }

  function restoreOriginals() {
    subLabels.forEach(sl => {
      sl.textContent = sl.dataset.original || sl.textContent;
    });
  }

  function updateByMode(mode) {
    const subscriptionItem = document.querySelector("#subscription_item");
    if( subscriptionItem ){      
      subscriptionItem.setAttribute("disabled", "true");
    }

    if (mode === 'subscription') {
      if( subscriptionItem ){
        subscriptionItem.removeAttribute("disabled");
      }
  ...
  // --- INITIAL UPDATE ---
  const checkedInput = document.querySelector('#juo-subscription-widget input[name="mode"]:checked');
  if (checkedInput) updateByMode(checkedInput.value);

  // --- LISTEN FOR MODE CHANGES ---
  document.querySelector('#juo-subscription-widget')?.addEventListener('change', function(e) {
      if (e.target.name === 'mode') {
        updateByMode(e.target.value);
      }
    });
});
...
// // Assuming there is an element with class "scoop_prod"
// var scoopProdElement = document.querySelector('.scoop_prod');

// // Assuming the form has ID "cart_rel-add-to-cart-form"
// var scoop_form = document.getElementById('cart_rel-add-to-cart-form');

// // Check if both the scoopProdElement and form exist
// if (scoopProdElement && scoop_form) {
//     // Add click event listener to scoopProdElement
//     scoopProdElement.addEventListener('click', function(event) {
//         // Prevent default behavior of the click event (e.g., following a link)
//         event.preventDefault();
...
//     });
// } else {
//     console.error('Element with class "scoop_prod" or form with ID "cart_rel-add-to-cart-form" not found.');
// }
...
function fetchCart(data) {
  if (location.href.indexOf('/cart') != -1) {
    changeCart(data.item_count);
  } else {
    setTimeout(function(){
     new theme.CartDrawer();
     document.querySelector("#CartDrawer").classList.remove("loading_cart");
    }, 500);
  }
}

function changeCart(cart_count) {
  fetch('/?section_id=cart-ajax')
    .then(response => response.text())
    .then(data => {
      const respHtml = new DOMParser().parseFromString(data, 'text/html');
      if (cart_count != 0) {
        document.querySelector(`[data-cart-element]`).classList.remove('is-empty');
        let respoData = respHtml.querySelector(`#CartPageForm`).innerHTML;
        document.querySelector(`#CartPageForm`).innerHTML = respoData;
      } else {
        document.querySelector(`[data-cart-element]`).classList.add('is-empty');
      }
    })
    .catch(error => {
      console.error(error);
...
function removeItem(keysToRemove, callback) {
  if (!keysToRemove || Object.keys(keysToRemove).length === 0) return;

  document.querySelector("#CartDrawer")?.classList.add("loading_cart");
  $.ajax({
    url: "/cart/update.js",
    method: "POST",
    contentType: "application/json",
    data: JSON.stringify({ updates: keysToRemove }),
    dataType: "json",
    success: function (data) {
      if (data.item_count === 0) {
        //console.log("Cart is now empty!");
        if (window.location.href.includes("recycled-measuring-scoop")) {
          //console.log("Refreshing page since cart is empty and URL matches...");
          location.reload();
        }
      }
...
function changeItemNew(dataToRemove, callback) {
...
$(document).ready(function() {  
  // SCOOP ADD
  $('body').on('click', '.scoop_prod', function(event) {
  ...
  const sachetHandles = [
    'organic-whey-protein-cookies-cream-one-per-flavour-25g',
    'organic-whey-protein-madagascan-vanilla-one-per-flavour-25g',
    'organic-whey-protein-pure-unflavoured-one-per-flavour-25g',
    'organic-whey-protein-chocolate-cacao-maca-one-per-flavour-25g',
    'organic-whey-protein-banana-lucuma-one-per-flavour-25g',
    'organic-whey-protein-coffee-one-per-flavour-25g'
  ];
  ...
  // $('body').on('click', '.sachet_line_item', async function  (e) {
  // e.preventDefault();

  window.freeSachetItemAddToCart = async function (variantId) {
    if(!variantId) return;
    ...
    drawer?.classList.add('loading_cart');
    ...
    try {    
      ...
      cart.items.forEach(item => {
        ...
        if (sachetHandles.includes(handle)) {
          updates[item.key] = 0; // remove
        }
      });
      ...
      window.theme?.CartDrawer && new window.theme.CartDrawer();
      //document.dispatchEvent(new CustomEvent("cart:refresh"));

    } catch (err) {
      console.error('Sachet replace error:', err);
    } finally {
      drawer?.classList.remove('loading_cart');
      isProcessing = false;
      //new theme.CartDrawer();
    }
  };

  // $('body').on('click', '.sachet_line_item', function(event) {
  //   event.preventDefault(); // prevent default form submission
  //   document.querySelector("#CartDrawer").classList.add('loading_cart');
  //     var id = $(this).attr("id")
  //     $.ajax({
  ...
  // vanila upsell call
  $('body').on('click', '.vanila_upsell', function(event) {
    event.preventDefault(); // prevent default form submission
      var id = $(this).attr("id")
    $.ajax({
      type: 'POST',
  ...
  $('body').on('click', '.quantity-decrement', function() {
      ...
      var t = $(this).siblings('.quantity');
      var quantity = parseInt($(t).val());
      // Remove the condition to allow quantity to go below 1
      $(t).val(quantity - 1);
  });
  ...
  $("body").on("click", ".remove-btn", function (e) {
    ...
    const itemKey = $(this).closest(".cart__item").data("item_key");
    if (itemKey) {
      const updates = { [itemKey]: 0 };
      removeItem(updates, function (cartData) {
        if (window.el_product.length != 0 && window.scoop_product != '') {
          const scoopTargetItem = cartData.items?.find(item => item.id === parseInt(window.scoop_product)),
                hasBoxItem = cartData.items?.some(item => item.properties && item.properties._box), 
                hasElProductItem = cartData.items?.some(item => window.el_product.includes(item.product_id));

          // NEW LOGIC: Only allow EXACT 400g or 1.2kg
          const hasValidWeightProduct = cartData.items?.some(item => {          
            if (item.id === parseInt(window.scoop_product)) return false;

            const variantTitle = item.variant_title?.trim().toLowerCase();
            return variantTitle === '400g' || variantTitle === '1.2kg';
          });

          // if (typeof scoopTargetItem != 'undefined' && !hasBoxItem && !hasElProductItem) {
          //   removeItem({ [scoopTargetItem.key]: 0 });
          // } else {
          //   fetchCart(cartData);
          // }

          if (typeof scoopTargetItem != 'undefined' && !hasValidWeightProduct) {
            removeItem({ [scoopTargetItem.key]: 0 });
          } else {
            fetchCart(cartData);
          }
  ...
  $("body").on("click", ".bundle_remove-btn", function (e) {
    e.preventDefault();

    const uniq = $(this).closest(".cart__item-box").find(".box__subitem").data("properties-uniq");
    const updates = {};

    if (uniq) {
      $(".box__subitem").each(function () {
        if ($(this).data("properties-uniq") === uniq) {
          updates[$(this).data("item_key")] = 0;
        }
      });

      // remove bundle items first
      removeItem(updates, function (cartData) {
        // after bundle removal, check for target item
        const targetItem = cartData.items?.find(item => item.id === 42390727917611);
        if (targetItem) {
          removeItem({ [targetItem.key]: 0 });
        } else {
          fetchCart(cartData);
        }   
        window.handleFreeItems();  
      });
    }
  });
  ...
  $('body').on('click', '.btn-minus', function(event) {
    event.preventDefault();
    var qty = $(this).closest('.qty-box').find('input').val();    
    var currentVariant = $(this).closest('.qty-box').find('input').attr('data-quantity-variant-id');
    const changeItemData = {
      quantity: qty,
      id: currentVariant
    }
    changeItemNew(changeItemData, function (cartData) {
      if (window.el_product.length != 0 && window.scoop_product != '') {
        const scoopTargetItem = cartData.items?.find(item => item.id === parseInt(window.scoop_product)),
              hasBoxItem = cartData.items?.some(item => item.properties && item.properties._box), 
              hasElProductItem = cartData.items?.some(item => window.el_product.includes(item.product_id));
  ...
  $('body').on('click', '.cart_pg_btn-minus', function(event) {
    event.preventDefault();
    var qty = $(this).closest('.qty-box').find('input').val();
    var currentVariant = $(this).closest('.qty-box').find('input').attr('id');
    const changeItemData = {
      quantity: qty,
      id: currentVariant
    }
...
const reloadButtons = document.querySelectorAll('.reload_remove');
let lastTwoClicks = [];

reloadButtons.forEach(btn => {
  btn.addEventListener('click', function () {
    lastTwoClicks.push(this);
    console.log('lastTwoClicks.length',lastTwoClicks.length)
    // keep only last 2 clicks
    if (lastTwoClicks.length > 2) lastTwoClicks.shift();

    // check if both have 'bundle_var' class
    if (
      lastTwoClicks.length === 2 &&
      lastTwoClicks.every(el => el.classList.contains('bundle_var'))
    ) {
      location.reload();
    }
  });
...
/* ===== CONFIG ===== */
const SCOOP_VARIANT_ID = 42390727917611;

/* ===== HELPER ===== */
async function isScoopInCart() {
  const cart = await fetch('/cart.js').then(r => r.json());
  return cart.items.some(item => item.variant_id == SCOOP_VARIANT_ID);
}

/* ===========================
   FETCH INTERCEPT (JSON + FORM)
=========================== */
(function(){
  const sachetHandles = [
    'organic-whey-protein-cookies-cream-one-per-flavour-25g',
  ...
  async function validateCart() {
    ...
    try {
      ...
      /* This global used for the check the Cart total to show sachet item on cart drawer */
      //window.handleFreeItems(cart);


      const scoopItem = cart.items.find(
        item => item.variant_id == SCOOP_VARIANT_ID
      );

      const hasEligibleProduct = cart.items.some(item => {
        return (item.options_with_values || []).some(opt => {
          const value = (opt.value || '')
            .toLowerCase()
            .replace(/\s+/g, '');

          return value === '400g' || value === '1.2kg';
        });
      });

      // if (cart.total_price === 0 && scoopItem) {
      const validHandles = [
        'pure-vanilla-bundle',
        'taste-them-all-bundle',
        'whey-protein-discovery-packed',
        'choco-lover-s-bundle'
      ];

      const hasTargetProduct = cart.items.some(item =>
        validHandles.includes(item.handle)
      );
      // console.log("hasEligibleProduct ==>");
      // console.log(hasEligibleProduct);
      // // if (cart.total_price === 0 && scoopItem) {
      // if (scoopItem && (cart.total_price === 0 || !hasEligibleProduct)) {


      // Free sachet item automatically add to cart cart total 50 or more
      // const VARIANT_ID = 43895053582379;
      // const cartTotal = cart.original_total_price / 100;

      // if (cartTotal < threshold) return;
      //   console.log(`cartTotal ${cartTotal}`);
      //   console.log(`threshold ${threshold}`);
      //   //const alreadyInCart = cart.items?.some(item => item.id === VARIANT_ID);
      //   const alreadyInCart = cart.items.some(item =>
      //     sachetHandles.includes(item.handle)
      //   );
      //   console.log(cart);
      ...
      //   }
        ...
        if (scoopItem && cart.total_price !== 0 && !hasTargetProduct && !hasEligibleProduct) {
          ...
          //alert("Free product requires another paid product in cart.");
          location.reload();
        }
    } catch(e){
  ...
  function disableCheckout() {
    const buttons = document.querySelectorAll('#MainContent .cart__checkout');
  ...
  window.fetch = async function(url, options = {}) {
    ...
    try {
      ...
      if(
        ...
        //triggerFreeSachetSync();
        const alreadyInCart = await isScoopInCart();
        if(alreadyInCart){

          let bodyData = options.body;
          let containsScoop = false;

          // JSON payload
          if(bodyData && typeof bodyData === 'string' && bodyData.trim().startsWith('{')) {
            try {
              const json = JSON.parse(bodyData);
              if(json.items){
                json.items.forEach(item=>{
                  if(item.id == SCOOP_VARIANT_ID) containsScoop = true;
                });
              }
              if(json.id == SCOOP_VARIANT_ID) containsScoop = true;
            } catch(e){}
          }
          ...
          if(bodyData && typeof bodyData === 'string'){
            ...
            if(params.get('id') == SCOOP_VARIANT_ID) {
              containsScoop = true;
            }
            for(const key of params.keys()){
              const match = key.match(/updates\[(\d+)\]/);
              if(match && match[1] == SCOOP_VARIANT_ID){
                containsScoop = true;
              }
            }
          }
          ...
          if(containsScoop){
            alert("This free scoop can only be added once per cart.");
            return Promise.reject("Blocked duplicate scoop add");
          }
...
/* cart threshold validation code: start */
  // const sachetHandles = [
  //   'organic-whey-protein-cookies-cream-sachets',
  //   'organic-whey-protein-madagascan-vanilla-sachets',
  //   'organic-whey-protein-pure-unflavoured-sachets',
  //   'organic-whey-protein-chocolate-cacao-maca-sachets',
  //   'organic-whey-protein-banana-lucuma-sachets',
  //   'organic-whey-protein-coffee-sachets'
  // ];

  

  document.addEventListener('change', function (event) {
    const select = event.target.closest('.sachet_items');

    if (select) {
      const selectedValue = select.value;
      const imageURL = select.options[select.selectedIndex].dataset.image_url;      
      const selectedText = select.options[select.selectedIndex].text;
      const offerBox = select.closest(".offer-box");
      // const upsell = offerBox.querySelector(".sachet_vanila_upsell");
      //   upsell['id'] = selectedValue;

      let sachetImage = document.querySelector(".sachet_flavour_image");  

      if( sachetImage && imageURL !='' ){
        document.querySelector(".sachet_flavour_image").src = imageURL;
      }  
      
      window.freeSachetItemAddToCart(selectedValue);
    }
  });

  // document.addEventListener('cart:updated', function (event) {
  //   const cart = event.detail.cart;
  //   handleFreeItems(cart); drawer drawer--cart drawer--cart--static drawer--is-open loading_cart
  // });

  let freeItemTimeout = null;

  window.handleFreeItems = async (cart) => {
    console.log("Handle Free Items");
    
    const res = await fetch('/cart.js');
    const carted = await res.json();
    cart = carted;
    console.log("Handle free cart");
    console.log(cart);
     // clear previous pending execution
    if (freeItemTimeout) {
      clearTimeout(freeItemTimeout);
    }
    
      let MatcheItemsRemove = false;
      let LineKey = "";

      // let cartTotal = cart.total_price / 100;
      let cartTotal = cart.original_total_price / 100;
      if (cartTotal >= threshold) return;

        cart.items.forEach(item => {
          if (sachetHandles.includes(item.handle) && !item.properties?._box ) {
            MatcheItemsRemove = true;
            LineKey = item.key;
          }
        });
      console.log("MatcheItemsRemove ==>");
      console.log(MatcheItemsRemove);

        if( MatcheItemsRemove && LineKey !=""){
  ...
  let isHandlingFreeItems = false;
  let cartPromise = null;

  async function newGetCart() {
    if (!cartPromise) {
  ...
  window.handleFreeItems = async (cart) => {
    if (isHandlingFreeItems) {
      console.log("Skipped: already processing");
      new theme.CartDrawer();
      return;
    }

    isHandlingFreeItems = true;

    try {
      console.log("Handle Free Items");
      // Use passed cart if valid, otherwise fetch
      if (!cart || !cart.items) {
        cart = await newGetCart();
      }
      ...
      if (MatcheItemsRemove && LineKey !== "") {
        ...
        if (typeof fetchCart === 'function') {
          console.log("calling msg get.......");
          fetchCart(updatedCart);
        }
        // IMPORTANT: invalidate cart cache after mutation
        cartPromise = null;
      }
    } catch (err) {
      console.error("Free item sync error:", err);
    } finally {
       document.querySelector("#CartDrawer")?.classList.remove("loading_cart");
      isHandlingFreeItems = false;
    }
  };


  // const FREE_VARIANT_ID = 43895053582379;
  const FREE_VARIANT_ID = 43895055581227;
  const THRESHOLD = threshold; //Assign threshold in theme.liquid file

  let isProcessingFreeItem = false;

  async function getCartd() {
    return fetch('/cart.js').then(r => r.json());
  }

  async function updateCartd(updates) {
    return fetch('/cart/update.js', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ updates })
  ...
  async function syncFreeSachet() {
        console.log("calling mug here multiple time");
    if (isProcessingFreeItem) return;
    isProcessingFreeItem = true;

    try {
      const cart = await getCartd();
      
      if (!cart) return;

      const total = cart.original_total_price / 100;
      //const freeItem = cart.items.find(i => i.id === FREE_VARIANT_ID);
      const hasSachet = cart.items.some(item => {
        const handle = item.product_handle || item.handle;
        return sachetHandles.includes(handle);
      });

      if (total >= THRESHOLD && !hasSachet) {
        document.querySelector("#CartDrawer")?.classList.add("loading_cart");
        await addItemd(FREE_VARIANT_ID);
        document.querySelector("#CartDrawer")?.classList.remove("loading_cart");
      }

    //   if (total < THRESHOLD && hasSachet) {
    //     const updates = {};
    //     updates[freeItem.key] = 0;
    //     await updateCartd(updates);
    //  }
     ...
     window.theme?.CartDrawer && new window.theme.CartDrawer();
      ...
      console.error('Free sachet error:', e);
    } finally {
    // new theme.CartDrawer();
      isProcessingFreeItem = false;
    }
  }
  ...
  window.triggerFreeSachetSync = function() {    
