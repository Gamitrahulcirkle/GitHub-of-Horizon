      ...
      quantityChanged: function(evt) {
        ...
        theme.cart.changeItem(key, qty)
          .then(function(cart) {
            ...
            this.buildCart();
            console.log("cart_update buildcart");
  ...
  theme.AjaxProduct = (function() {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      ...
      success: function(product) {
        ...
        window.triggerFreeSachetSync(product);
      },
  ...
  window.handleFreeItems = async (cart) => {
    ...
    try {
      ...
      if (MatcheItemsRemove && LineKey !== "") {
        console.log("callling change.js for free sachet items");
         document.querySelector("#CartDrawer")?.classList.add("loading_cart");
...
//======== Free Sachet item added/remove: Start =======//
  ...
  async function syncFreeSachet(cart) {
    if (!cart) return;
    ...
    if (total < THRESHOLD && hasSachet) {
      const item = cart.items.find(i =>
        sachetHandles.includes(i.handle)
      );
  ...
  document.addEventListener('cart:updated', function (event) {
    clearTimeout(testSyncTimer);

    testSyncTimer = setTimeout(async () => {
      if (isProcessing) return;
      isProcessing = true;

      try {
        const cart = event.detail.cart;
        await syncFreeSachet(cart);
      } catch (e) {
        console.error(e);
      } finally {
        isProcessing = false;
...
//======== Free Sachet item added/remove: End =======//
